...
**小学生が音楽を通して、試行錯誤と表現を学ぶための創作環境。**

音楽は手段であって主題ではありません。学ぶのは「自分の意図を持ち、直しながら近づけていくこと」です。
価値の単位は完成した曲ではなく**試した回数**——だからこのアプリは、
子どもが**曲を手に入れる**ためではなく、**自分で作れるようになる**ために設計されています。

低い床（数秒で音が出る）と高い天井（近づいていく先がある）を両立させ、
その間にある制約は、子どもが育つにつれて自分で外していく足場として扱います。

設計上の非目標（意図的にやらないこと）は [CLAUDE.md](CLAUDE.md) を参照してください。
...
- **お題（チャレンジ）**: 「あめの音」「おばけが ちかづいてくる」など**正解のないお題**。採点せず、曲に応じて画面が反応する。同じお題に対する答えのばらつき自体が学び
- **おとのいきもの**: 3度や反復などの音楽的な関係を、**教えずに**発見できる仕組み。見つけると「生き物との出会い」として現れる。用語は体験の後から任意で確認できる。協和＝正解／不協和＝失敗とは扱わない（不協和にも休符にも生き物がいる）
- **おとのずかん** (`/discoveries`): 出会った生き物の図鑑。未発見はシルエット。**採点も順位付けもしない**
- **いっしょにつくる**: 出会った生き物から1体を選ぶと、編集画面にふわふわ浮いて付いてくる（ドラッグで移動可）
- **先生ページ** (`/teacher`): 先生のみ。誰がどの曲を作ったかの確認と、不適切な曲の非表示（可逆）。児童には保存時に「先生に見える」ことを明示
- **設定パネル**: テンポ・音色・音域・ブロック数・黒鍵・使う音・リセットを「⚙ せってい」パネルに集約（リセットは確認ステップ付き）
- **曲の保存・共有**: Supabase に保存し、みんなの曲一覧ページで共有
- **おとのいきもの**: 音の組み合わせやリズムを再生中に発見すると生き物と出会える（**初回だけ**演出。既に会った生き物は静かに見守る）
...
|------|------|
| `/` | 作曲エディタ |
| `/discoveries` | おとのずかん（出会った生き物のアルバム） |
| `/songs` | みんなの曲一覧（SNS 風フィード） |
| `/teacher` | 先生ページ（先生のみ。誰が作ったかの確認と非表示） |
...
│   │   ├── discovery/ # 出会い演出・図鑑エントリ UI
│   │   ├── companion/ # いっしょにつくる（同伴キャラ）UI
│   │   └── challenges/# お題の選択・演出 UI
│   ├── discoveries/  # おとのずかん（アルバム）ページ
│   ├── songs/        # みんなの曲一覧ページ
│   ├── teacher/      # 先生ページ（先生のみ・モデレーション）
...
├── discovery/        # 何を見つけたかの判定（純粋ロジック・DOM/Audio 非依存）
│   └── rules/        # 和音・メロディ・リズム別の検出ルール
├── creatures/        # 見つけたものを「どう見せるか」（絵・名前・演出）。検出はしない
├── challenges/       # お題のカタログと、曲に応じた場面の反応
